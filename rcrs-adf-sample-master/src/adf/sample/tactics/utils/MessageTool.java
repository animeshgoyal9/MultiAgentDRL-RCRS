...
public class MessageTool {
	...
	public MessageTool(ScenarioInfo scenarioInfo, DevelopData developData) {
		...
		this.sendingAvoidTimeReceived = developData.getInteger("sample.tactics.MessageTool.sendingAvoidTimeReceived",
				3);
		this.sendingAvoidTimeSent = developData.getInteger("sample.tactics.MessageTool.sendingAvoidTimeSent", 5);
		this.sendingAvoidTimeClearRequest = developData
				.getInteger("sample.tactics.MessageTool.sendingAvoidTimeClearRequest", 5);
	...
	public void reflectMessage(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
		...
		for (CommunicationMessage message : messageManager.getReceivedMessageList(StandardMessage.class)) {
			System.out.println("Stardard Message: " + message);
	...
	public void sendInformationMessages(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
			MessageManager messageManager) {
		Set<EntityID> changedEntities = worldInfo.getChanged().getChangedEntities();
				...
				if (!(isRecentlyReceived(agentInfo, entityID))) {
					...
					case ROAD:
						...
						if (isNonBlockadeAndNotReceived(road)) {
							message = new MessageRoad(true, StandardMessagePriority.LOW, road, null, true, false);
						}
						break;
					case BUILDING:
	...
	public void sendRequestMessages(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
			MessageManager messageManager) {
		if (agentInfo.me().getStandardURN() == AMBULANCE_TEAM || agentInfo.me().getStandardURN() == FIRE_BRIGADE) {
			...
			if (positionEntity instanceof Road) {
				...
				if (isSendRequest && ((currentTime - this.lastSentTime) >= this.sendingAvoidTimeClearRequest)) {
					...
					messageManager
							.addMessage(new CommandPolice(true, null, agent.getPosition(), CommandPolice.ACTION_CLEAR));
				}
			}
		}
	}

	private void updateInfo(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
			MessageManager messageManager) {
	...
	private boolean isOnFireOrWaterDameged(Building building) {
		final List<StandardEntityConstants.Fieryness> ignoreFieryness = Arrays
				.asList(StandardEntityConstants.Fieryness.UNBURNT, StandardEntityConstants.Fieryness.BURNT_OUT);
